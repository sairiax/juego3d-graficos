...
const modelsToLoad = 8; // player model + 2 anims + 3 enemies + trees (as one) + meteor/lava
let modelsLoaded = 0;
let isInitialized = false;
// Meteor/Lava model template
let meteorLavaTemplate;
let treeTemplate; // Global for regeneration
let streeTemplate; // New tree template
let hasTreeTemplates = false; // Flag to check if at least one tree template loaded successfully
...
function placeTrees(radio, num) {
    ...
    while (placedTrees < num) {
        ...
        const useStree = Math.random() < 0.5;
        let template = useStree ? streeTemplate : treeTemplate;
        if (!template) {
            template = treeTemplate || streeTemplate;
        }
        if (!template) continue;
        const intendedStree = useStree;
        ...
        let treeOffset = CHARACTER_HEIGHT + scaleFactor * (intendedStree ? 0.1 : 1.333);
        tree.position.add(radial.clone().multiplyScalar(treeOffset));
        tree.userData = {
            isTree: true,
            radial: radial.clone(),
            offset: treeOffset,
            isCollidable: !intendedStree // true for normal trees, false for stree
        };
        scene.add(tree);
        staticObstacles.push(tree);
        placedTrees++;
    }
    console.log(`${num} trees placed`);
}
function regenerateTrees(num) {
    ...
    placeTrees(currentPlanetRadius, num);
...
function loadPlaneta(radio, texturaURL) {
    ...
    // Load tree models
    const totalTreeTypes = 2;
    let treesLoaded = 0;
    let successfulTreeTypes = 0;
    function onTreeTypeLoaded(success) {
        treesLoaded++;
        if (success) successfulTreeTypes++;
        if (treesLoaded === totalTreeTypes) {
            if (successfulTreeTypes === 0) {
                createProceduralTrees(radio, controls.numTrees);
            } else {
                placeTrees(radio, controls.numTrees);
                hasTreeTemplates = true;
            }
            modelLoaded();
        }
    }
    ...
    treeLoader.load('models/tree.glb', function (gltf) {
        ...
        onTreeTypeLoaded(true);
    }, undefined, function (error) {
        console.error('Error loading tree model:', error);
        onTreeTypeLoaded(false);
    });
    const streeLoader = new THREE.GLTFLoader();
    streeLoader.load('models/stree.glb', function (gltf) {
        streeTemplate = gltf.scene.clone();
        streeTemplate.traverse((child) => {
            if (child.isMesh) {
                child.castShadow = true;
                child.receiveShadow = true;
            }
        });
        streeTemplate.scale.set(3, 3, 3);
        console.log('Stree model loaded');
        onTreeTypeLoaded(true);
    }, undefined, function (error) {
        console.error('Error loading stree model:', error);
        onTreeTypeLoaded(false);
    });
}
...
function checkStaticCollisions(newPos) {
    for (let staticObj of staticObstacles) {
        ...
        if (staticObj.userData && staticObj.userData.isTree) {
            ...
            if (staticObj.userData.isCollidable === false) continue; // Skip collision for stree
        }
